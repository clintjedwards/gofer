...
We can highjack the concept of a extension. We create a notifications extension which monitors a specific pipeline.

- It can be used to monitor a pipeline you do not own and start your own pipeline based on that.
- It can be used to send you notifications about that pipeline, slack, email, whatever. This is different from the common
  task for like slack, since it doesn't extension on every event.
...
- Pipeline errors needs a bit of work, the idea isn't fully formed yet. We need a way to alert users that because of Gofer changes their
  ...
  - What types of errors do we need to account for?
    - A extension subscription has failed due to the extension being not found or unable to be contacted.
    - A pipeline's config for extension/commontask is invalid
    - A Gofer has uninstalled a extension/commontask that a pipeline previously depended on.
  - When we uninstall common tasks or extensions we can list all pipelines that currently use those, disable them and add an error.
  - Create an API side pipeline validate that uses the sdk validate but also implements some things the SDK cannot do, like for instance check that all the extensions mentioned in the pipeline config are registered
...
- The storage layer could benifit from more autogenerated code: https://docs.sqlc.dev/en/latest/tutorials/getting-started-postgresql.html
...
- Pipeline get needs to put more detail (list tasks, extensions, commontasks)
...
- Expand the CLI up command to actually walk the user through the deployment. Right now it just starts the deployment and walks away.
...
- The interval extension should create jitter of about 2-5 mins. During that time it can choose when to start counting to extension an event. This is so that when we restart the server all events don't perfectly line up with each other.
...
- Should extensions be able to pass "metadata" values back to pipelines that are secret?
...
- It is possible for a extension subscription to be disabled due to network error and the extension to still send it a successful event, but
  not understand that it wasn't successfully delivered. Overtime this might cause drift between what events extensions should actually be sending back.
...
- Update pipeline getting started documentation, extensions are handled differently now.
