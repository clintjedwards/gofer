...
func TestCRUDPipelineCustomTasks(t *testing.T) {
	path := tempFile()
	db, err := New(path, 200)
	if err != nil {
		t.Fatal(err)
	...
	err = db.InsertPipelineCustomTask(db, &task)
	if err != nil {
		t.Fatal(err)
	}

	taskList, err := db.ListPipelineCustomTasks(db, pipeline.Namespace, pipeline.ID, config.Version)
	if err != nil {
		t.Fatal(err)
	}
	...
	fetchedTask, err := db.GetPipelineCustomTask(db, namespace.ID, pipeline.ID, config.Version,
		task.ID)
	if err != nil {
		t.Fatal(err)
	}
