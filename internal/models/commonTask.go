...
func (c *CommonTaskRegistration) ToStorage() *storage.CommonTaskRegistration {
	...
	if c.RegistryAuth != nil {
		ctr.RegistryAuth = c.RegistryAuth.ToStorage()
	}
	...
	ctr.Variables = string(variables)

	return ctr
}

func (c *CommonTaskRegistration) FromStorage(storage *storage.CommonTaskRegistration) {
	var registryAuth RegistryAuth
	err := json.Unmarshal([]byte(storage.RegistryAuth), &registryAuth)
	if err != nil {
		log.Fatal().Err(err).Msg("error in translating from storage")
	}
	...
	c.Name = storage.Name
	c.Image = storage.Image
	c.RegistryAuth = &registryAuth
	c.Variables = variables
	c.Created = time.Now().UnixMilli()
	c.Status = CommonTaskStatus(storage.Status)
	c.Documentation = storage.Documentation
}
