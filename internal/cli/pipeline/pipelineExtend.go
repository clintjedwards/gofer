...
by using the '--setting' flag. You can find a list of the settingeters your extension needs by reading the documentation or
using the '--interactive' flag. The '--interactive' flag connects you to the extension and walks you through an install
process.

For example, the "interval" extension requires the subscribing pipeline to specify which time interval it would like to
be run on. The settingeter is called "every". So one might subscribe to the interval extension like so:

ex. gofer pipeline extend simple interval every_5_seconds -s every="5s"

Passing the config 'every="5s"' as the config settingeter required`,
...
func init() {
	cmdPipelineExtend.Flags().BoolP("interactive", "i", false, "Attempt to sub to extension by querying the extension for setting information.")
