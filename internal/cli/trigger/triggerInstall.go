...
var cmdTriggerInstall = &cobra.Command{
	...
	Short: "Install a trigger by image",
	Long: "Install a trigger by image.\n\n" +
		"Gofer allows two ways to install a trigger. The first is by configuration where you enter the trigger's required" +
...
1) Gofer attempts to run the trigger container locally and connects the user's terminal to stdout/in/err
...
3) The trigger container will attempt to install the trigger into Gofer on behalf of the user.`,
	Example: `$ gofer trigger install cron gchr.io/clintjedwards/gofer-containers/triggers/cron`,
	RunE:    triggerInstall,
	Args:    cobra.ExactArgs(3),
}

func init() {
	cmdTriggerInstall.Flags().StringP("host", "h", "", "URL of Gofer server")
	cmdTriggerInstall.Flags().StringP("user", "u", "", "The username needed for authentication to docker image repository")
	cmdTriggerInstall.Flags().StringP("pass", "p", "", "The password needed for authentication to docker image repository")
...
func triggerInstall(cmd *cobra.Command, args []string) error {
	...
	containerConfig := &container.Config{
		...
		Env: []string{
			fmt.Sprintf("GOFER_TRIGGER_INSTALLER_KIND=%s", kind),
			fmt.Sprintf("GOFER_TRIGGER_INSTALLER_SERVER_HOST=%s", host),
			fmt.Sprintf("GOFER_TRIGGER_INSTALLER_TOKEN=%s", cl.State.Config.Token),
			fmt.Sprintf("GOFER_TRIGGER_INSTALLER_IMAGE_USER=%s", user),
			fmt.Sprintf("GOFER_TRIGGER_INSTALLER_IMAGE_PASS=%s", pass),
		},
		AttachStdout: true,
		AttachStderr: true,
		AttachStdin:  true,
	...
	_ = client.ContainerRemove(ctx, fmt.Sprintf("gofer_trigger_installer_%s", kind), removeOptions)

	createResp, err := client.ContainerCreate(ctx, containerConfig, hostConfig, nil, nil,
		fmt.Sprintf("gofer_trigger_installer_%s", kind))
	if err != nil {
		cl.State.Fmt.PrintErr(err)
		cl.State.Fmt.Finish()
		return err
	...
	case <-wait:
		...
		cl.State.Fmt.PrintSuccess(fmt.Sprintf("Successfully installed trigger %q", kind))
		return nil
	case err = <-waitErr:
