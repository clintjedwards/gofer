...
func main() {
	err := sdk.NewPipeline("common_task", "Common Task Pipeline").
		WithDescription("This pipeline shows off the common tasks feature of Gofer. Common Tasks allow administrators to" +
		...
		WithTasks(sdk.NewCommonTask("debug", "debug_task")).Finish()
	if err != nil {
		log.Fatal(err)
	}
}
