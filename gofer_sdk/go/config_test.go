	...
	err := NewPipeline("simple_test_pipeline", "Simple Test Pipeline").
		...
		WithTasks([]Task{
			*NewTask("simple_task", "ubuntu:latest").
				...
				WithExec("/bin/bash", `echo Hello from Gofer!`),
		}).
		Finish()
	if err != nil {
		panic(err)
