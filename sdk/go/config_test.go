...
func TestInvalidPipelineCyclical(t *testing.T) {
	taskA := NewTask("task_a", "").WithDependsOnOne("task_b", RequiredParentStatusAny)
	taskB := NewTask("task_b", "").WithDependsOnOne("task_c", RequiredParentStatusAny)
	taskC := NewTask("task_c", "").WithDependsOnOne("task_a", RequiredParentStatusAny)

	err := NewPipeline("invalid_pipeline", "").WithTasks([]Task{
		*taskA, *taskB, *taskC,
	}).Finish()
